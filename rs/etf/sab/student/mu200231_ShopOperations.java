...
public class mu200231_ShopOperations implements ShopOperations {
	...
	@Override
	public int createShop(String p0, String p1) {
		try(
		...
		ResultSet rs1 = null;
		try(
				PreparedStatement ps=connection.prepareStatement("INSERT INTO Prodavncia values(?,0,?)");
						PreparedStatement ps1=connection.prepareStatement("select * from grad where naziv=?");
				){
					ps1.setString(1, p1);
					 rs1=ps1.executeQuery();
					if(rs1.next())ps.setInt(2, rs1.getInt(1));
					else return -1;
					ps.setString(1, p0);
	...
	public int getCity(int p0) {
		ResultSet rs1=null;
		try(
				PreparedStatement ps=connection.prepareStatement("select * from Prodavnica where IDprod=?)")
				){
					ps.setInt(1, p0);
